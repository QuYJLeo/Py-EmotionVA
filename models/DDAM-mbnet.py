...
class DDAMNet(nn.Module):
    """
    Dual-Distribution Attention Module Network with MobileNetV2 backbone.
    Uses pretrained MobileNetV2 as feature extractor with multiple attention heads.

    Args:
        num_class: Number of emotion classes (default: 7)
        num_head: Number of attention heads (default: 2)
        pretrained: Whether to load pretrained MobileNetV2 weights (default: True)
    """
    def __init__(self, num_class=7,num_head=2, pretrained=True):
        ...
        if pretrained:
            net.load_state_dict(torch.load("./pretrained/mobilenet_v2-b0353104.pth"))
        self.features = net.features

        # Additional convolution layer to reduce channels from 1280 to 512
        self.features.append(nn.Sequential(
            nn.Conv2d(1280, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False),
